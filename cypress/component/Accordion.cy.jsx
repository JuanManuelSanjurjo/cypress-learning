...
describe('Accordion.cy.jsx', () => {
  ...
  it("should render the Accordion component", () => {
    cy.getDataTest("accordion-item1").should("exist")
    cy.getDataTest("accordion-wrapper").within(()=> {
        cy.get("[data-test^='accordion-item']").should("have.length", 8)
      })

    cy.contains("Your tests will exist").should("not.be.visible")
    cy.getDataTest("accordion-item1").within(() => cy.get("[role=button]").click())
    cy.contains("Your tests will exist").should("be.visible")
    cy.getDataTest("accordion-item1").within(() => cy.get("[role=button]").click())
    cy.contains("Your tests will exist").should("not.be.visible")
